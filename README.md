...
### Merge `4_mapping/` and `5_aDNA_characteristics/` from different species into single diretory

Since samples from different species require alignment to their respective reference genomes (see Table 2 in manuscript), the pipeline should be run separately for each species:

1. Run the prep and main scripts once per species, using species-specific project names and reference genomes
2. After processing, merge the `4_mapping/` and `5_aDNA_characteristics/` outputs into single directories for downstream R analysis
```bash
# Example: After running pipeline separately for each species
mkdir -p Combined_analysis/4_mapping
mkdir -p Combined_analysis/5_aDNA_characteristics

# Merge outputs from each species-specific run
cp Hordeum_vulgare/4_mapping/*.flagstat.log Combined_analysis/4_mapping/
cp Hordeum_spontaneum/4_mapping/*.flagstat.log Combined_analysis/4_mapping/
cp Oryza_latifolia/4_mapping/*.flagstat.log Combined_analysis/4_mapping/
# ... repeat for other species

cp -r Hordeum_vulgare/5_aDNA_characteristics/* Combined_analysis/5_aDNA_characteristics/
cp -r Hordeum_spontaneum/5_aDNA_characteristics/* Combined_analysis/5_aDNA_characteristics/
cp -r Oryza_latifolia/5_aDNA_characteristics/* Combined_analysis/5_aDNA_characteristics/
# ... repeat for other species
```

The R analysis scripts (starting with `aDNA_Dmg_Script00_collate_screening_results.r`) expect all samples to be in unified `4_mapping/` and `5_aDNA_characteristics/` directories.
